...
public class SpotifyTokenManager {
	...
	protected synchronized String requestAccessToken() {
		...
		String credentials = new String(Base64.getEncoder()
			.encode((SPOTIFY_CLIENT_ID + ":" + SPOTIFY_CLIENT_SECRET).getBytes(StandardCharsets.UTF_8)));
		...
		try {
			...
			response = restTemplate.exchange("https://accounts.spotify.com/api/token", HttpMethod.POST, entity,
				String.class);
